...
Sprite2D::Sprite2D(Actor* _pOwner, Texture& _pTexture, uint8_t _drawOrder):
	Component(_pOwner), mTexture(_pTexture), mDrawOrder(_drawOrder), mTextureWidth(mTexture.GetWidth()), mTextureHeight(_pTexture.GetHeight())
{
...
void Sprite2D::Draw(Renderer& _pRenderer)
{
	...
	_pRenderer.DrawSprite(*pOwner, mTexture, Rectangle(), origin, Renderer::Flip::None);
}
